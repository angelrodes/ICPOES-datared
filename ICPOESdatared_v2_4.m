...
disp(['Sample,Location,Analyte,Element,Wavelength,ICPsignal,ICPsignal_error,NOMINAL_CONCENTRATIONS','RSD'])
linecount=0;
for row=sampleindex
    analytecount=0;
    for Analyte=analytes
        ...
        dsignalcolumn2=find(strcmp(dystring2,data(1,:))&strcmp(Analyte,data(3,:)));
...
if sum(isnan(errors))>0.5*numel(errors) % if no STD
    warning('Most of the Raw.STD emapty! Using Raw.Average*Raw.RSD/100 instead...')
